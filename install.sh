...
configure_l2tp() {
  ...
  # VPN_SUBNET = 10.10.0.0/24 → l2tp_local=10.10.0.1, l2tp_pool=10.10.0.200-10.10.0.250
...
configure_firewall() {
  ...
  # NAT masquerade for VPN clients (IKEv2 tunnel pool + L2TP ppp interfaces)
  _ipt_add -t nat POSTROUTING \
    -s "$VPN_SUBNET" -o "$WAN_IFACE" -j MASQUERADE \
    -m comment --comment "${TAG}"

  # Forward: VPN subnet out
  _ipt_add FORWARD \
    -s "$VPN_SUBNET" -j ACCEPT \
    -m comment --comment "${TAG}"

  # Forward: return traffic for established/related connections
  _ipt_add FORWARD \
