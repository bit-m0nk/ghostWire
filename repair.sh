...
# ── 6b. dnsmasq health check & auto-repair ───────────────────────────────────
hdr "dnsmasq (DNS Ad-Blocking)"
if ! command -v dnsmasq &>/dev/null; then
  info "dnsmasq not installed — installing..."
  apt-get install -y dnsmasq 2>/dev/null && log "dnsmasq installed" || err "dnsmasq install failed"
fi

# Ensure the log directory exists
mkdir -p /var/log/dnsmasq /etc/dnsmasq.d/ghostwire /opt/ghostwire/blocklists

# Write config if it's missing or doesn't have log-queries
if ! grep -q "log-queries" /etc/dnsmasq.conf 2>/dev/null; then
  info "Writing /etc/dnsmasq.conf with log-queries enabled..."
...
DNSMASQ_EOF
  log "/etc/dnsmasq.conf written"
fi

systemctl enable dnsmasq 2>/dev/null || true
systemctl restart dnsmasq 2>/dev/null && log "dnsmasq running" || err "dnsmasq failed to start"
