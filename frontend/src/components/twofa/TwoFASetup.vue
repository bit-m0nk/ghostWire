...
async function startSetup() {
  ...
  try {
    ...
    //         then fetch the SVG via the same axios instance so the
    //         Authorization header interceptor is always applied.
    qrSvg.value     = ''
    ...
    setupError.value   = e.response?.data?.detail || e.message || 'Setup failed — please try again'
  } finally {
