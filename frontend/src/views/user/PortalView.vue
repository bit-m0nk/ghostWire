      ...
      <section v-show="activeSection === 'overview'" class="section-body">
        ...
        <div class="card mt-4" v-if="serverInfo">
          ...
          <div class="server-info-grid">
            ...
            <div class="si-row"><span class="si-key">Ports</span><code class="si-val">UDP 500, 4500</code></div>
        ...
        <div v-if="configsLoaded">
          ...
          <div v-if="activeOs==='windows'" class="method-grid">
            <div class="method-card recommended">
              ...
              <div class="steps"><p>1. Download script below</p><p>2. Right-click → <b>Run as Administrator</b></p><p>3. Settings → Network → VPN → connect</p></div>
          ...
          <div v-else class="sessions-list">
            <div v-for="sess in mySessions" :key="sess.id" class="session-card">
              ...
              <div class="sess-bytes">
                <div class="sb-row"><span class="sb-in">↓</span><span>{{ fmtBytes(sess.bytes_in) }}</span></div>
                <div class="sb-row"><span class="sb-out">↑</span><span>{{ fmtBytes(sess.bytes_out) }}</span></div>
              </div>
            </div>
          </div>
        </div>
...
const totalBytesIn  = computed(() => mySessions.value.reduce((a, s) => a + (s.bytes_in  || 0), 0))
const totalBytesOut = computed(() => mySessions.value.reduce((a, s) => a + (s.bytes_out || 0), 0))
...
async function startSetup() {
  ...
  try {
    ...
    qrSvg.value = ''; qrVisible.value = true
    const qrResp = await api.get('/2fa/setup/qr', { responseType: 'text' })
    qrSvg.value = qrResp.data
  } catch (e) {
...
.method-card { background:var(--bg2); border:1px solid var(--border); border-radius:var(--radius); padding:16px; position:relative; }
