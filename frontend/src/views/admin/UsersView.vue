          ...
          <tr v-for="u in filtered" :key="u.id">
            ...
            <td>
              <div class="action-icons">
                <button class="icon-action" @click="openEdit(u)" title="Edit user">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                </button>
                <button class="icon-action" @click="openResetPanel(u)" title="Reset panel password">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg>
                </button>
                <button class="icon-action" @click="resetVpn(u)" title="Reset VPN password">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
                </button>
                <button v-if="u.totp_enabled" class="icon-action" @click="reset2fa(u)" title="Disable 2FA (lost device)">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2a10 10 0 100 20A10 10 0 0012 2z"/><path d="M4.93 4.93l14.14 14.14"/></svg>
                </button>
                <button class="icon-action danger" @click="deleteUser(u)" :disabled="u.is_admin" title="Delete user">
...
.icon-action:hover { background:var(--bg3); color:var(--text); border-color:var(--accent); }
.icon-action.danger { color:var(--text2); }
.icon-action.danger:hover { background:rgba(239,68,68,.1); color:var(--danger); border-color:var(--danger); }
.icon-action:disabled { opacity:.35; cursor:not-allowed; pointer-events:none; }
