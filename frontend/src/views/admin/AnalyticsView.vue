  ...
  try {
    const res = await api.post('/graphql', {
      query: `{ userSummaries(hours: ${selectedHours.value}) {
        user_id username total_queries blocked_count block_rate
      }}`
    })
    userSummaries.value = res.data?.data?.userSummaries || []
  } catch {
    userSummaries.value = []
  }
...
async function loadEvents() {
  ...
  try {
    ...
    if (!isAdmin.value || selectedUser.value) params.user_id = selectedUser.value || undefined
    const resp = (await api.get('/dns/events', { params })).data
    events.value      = resp.items
    totalEvents.value = resp.total
  } catch {}
