...
def activate_plugin(db: Session, slug: str) -> tuple[bool, str]:
    ...
    try:
        ...
        if router_module_name not in sys.modules:
            ...
            mod = importlib.reload(sys.modules[router_module_name])
        if hasattr(mod, "router"):
            _mounted_routers[slug] = mod.router
            # Mount the router into the running FastAPI app immediately —
            # no restart required. We import `app` lazily here to avoid a
            # circular import at module load time.
            try:
                from app.main import app as _fastapi_app
                # Only mount if not already registered (idempotent)
                existing_routes = {r.path for r in _fastapi_app.routes}
                prefix = f"/api/plugins/{slug}"
                if not any(r.startswith(prefix) for r in existing_routes):
                    _fastapi_app.include_router(
                        mod.router,
                        prefix=prefix,
                        tags=[f"Plugin:{slug}"],
                    )
                    log.info(f"Plugin '{slug}': router mounted at {prefix}/")
                else:
                    log.info(f"Plugin '{slug}': router already mounted at {prefix}/")
            except Exception as mount_err:
                log.warning(f"Plugin '{slug}': could not mount router at runtime: {mount_err}")
