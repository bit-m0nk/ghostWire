    ...
    for cfg in configs:
        ...
        if cfg.platform == "telegram":
            if cfg.tg_bot_token and cfg.tg_chat_id:
                error = _send_telegram(cfg.tg_bot_token, cfg.tg_chat_id, message)
            else:
                error = "Missing token or chat_id"

        elif cfg.platform == "discord":
            if cfg.discord_webhook_url:
                error = _send_discord(cfg.discord_webhook_url, message)
            else:
                error = "Missing webhook URL"

        elif cfg.platform == "slack":
            if cfg.slack_webhook_url:
                error = _send_slack(cfg.slack_webhook_url, message)
            else:
                error = "Missing webhook URL"
