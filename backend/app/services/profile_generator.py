...
"""
    ...
    return f"""# {brand} VPN — Windows Setup Script
# Run as Administrator (right-click → Run as administrator)
# PowerShell 5.0+  |  Windows 7/8/10/11 x64
...
try {{
    ...
    Write-Host "  Registry update skipped (non-fatal): $_" -ForegroundColor Yellow
}}
...
Write-Host "  OR: Click the network icon in taskbar → $VPNName"
Write-Host ""
Write-Host "  Username : $Username"
Write-Host "  Password : $Password"
Write-Host ""
Write-Host "  NOTE: If connection fails the first time, disconnect and try again." -ForegroundColor Yellow
Write-Host "  The registry change requires Windows to renegotiate the cipher." -ForegroundColor Yellow
Write-Host ""
